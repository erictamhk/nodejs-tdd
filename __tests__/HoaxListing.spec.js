...
describe("Listing All Hoaxes", () => {
  const addHoaxes = async (count = 10) => {
    const hoaxIds = [];
  ...
  const getHoaxes = () => {
    return request(app).get("/api/1.0/hoaxes");
  };
  ...
  it("returns only id, content, timestamp and user object having id, username, email, image in content array for each hoax", async () => {
    await addHoaxes(11);
    const response = await getHoaxes();
  ...
  it("returns fileAttachment having filename, fileType if Hoax has any", async () => {
    ...
    const attachmentKeys = Object.keys(hoax.fileAttachment);
    expect(attachmentKeys).toEqual(["filename", "fileType"]);
  });
...
describe("Listing Hoaxes of a User", () => {
  ...
  const addHoaxes = async (count = 10, userId) => {
    const hoaxIds = [];
    for (let i = 0; i < count; i++) {
      const hoax = await Hoax.create({
        content: `Hoax content ${i + 1}`,
  ...
  it("returns 404 when user dose not exist", async () => {
  ...
  `("return error object with $message for unknow user when leanguage is $language", async ({ language, message }) => {
  ...
  it("returns 5 hoaxes belongs to user in page content when there are totla 11 hoaxes", async () => {
    const user = await addUser();
