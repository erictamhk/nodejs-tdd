...
describe("Delete Hoax", () => {
  ...
  it("returns 403 when token is invalid", async () => {
    const response = await deleteHoax(5, { token: "abced" });
    expect(response.status).toBe(403);
  });

  it.each`
    language | message
    ${"hk"}  | ${hk.unauthroized_hoax_delete}
    ${"en"}  | ${en.unauthroized_hoax_delete}
  `(
  ...
  it("returns 403 when user tries to delete another user's hoax", async () => {
    ...
    const token = await auth({ auth: { email: user2.email, password: activeUser.password } });
